...
class WaitingForCatchUpDataProcessor : public BaseProcessor<cpp2::CatchUpResp> {
 public:
  ...
  void process(const cpp2::CatchUpDataReq& req) {
    ...
    auto ret = env_->kvstore_->part(spaceId, partId);
    if (!ok(ret)) {
      LOG(INFO) << "CatchUpDataState: part " << partId << " was not found";
    ...
    LOG(INFO) << "CatchUpDataState return code: " << static_cast<int32_t>(resp_.get_code())
              << ", status: " << static_cast<int32_t>(resp_.get_status())
              << ", snapshotRows: " << resp_.get_snapshotRows()
              << ", commitLogId: " << resp_.get_commitLogId();
    promise_.setValue(resp_);
    onFinished();
  }
