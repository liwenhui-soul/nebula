...
class AddListener final : public SingleDependencyNode {
 public:
  ...
  const HostAddr* metaListener() const {
    return metaListener_;
  }
...
class ShowListener final : public SingleDependencyNode {
 ...
 private:
  ShowListener(QueryContext* qctx, PlanNode* input, meta::cpp2::ListenerType type)
...
class CreateUser final : public CreateNode {
 public:
  ...
  const std::unordered_set<std::string>* ipWhitelist() const {
    return ipWhitelist_;
  }
...
class SignInSpaceService final : public SingleDependencyNode {
 public:
  ...
  meta::cpp2::ExternalSpaceServiceType type() const {
    return type_;
  }
