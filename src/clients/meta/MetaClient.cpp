...
bool MetaClient::loadData() {
  ...
  newMetaData->userIpWhitelistMap_ = std::move(userIpWhitelistMap_);
  newMetaData->metaListeners_ = std::move(metaListeners_);
...
Status MetaClient::checkIpWhitelistFromCache(const std::string& account,
                                             const std::string& clientIp) {
  if (!ready_) {
    return Status::Error("Meta service is not ready");
  }
  folly::rcu_reader guard;
  const auto& metadata = *metadata_.load();
  auto iter = metadata.userIpWhitelistMap_.find(account);
  if (iter == metadata.userIpWhitelistMap_.end() || iter->second.empty()) {
    return Status::OK();
  }
  return iter->second.find(clientIp) != iter->second.end()
             ? Status::OK()
             : Status::Error("ClientIp: %s is not in ip whitelist", clientIp.c_str());
}
...
MetaClient::getMetaListenerInfoFromCache(HostAddr host) {
  ...
  auto it = metadata.metaListeners_.find(host);
  if (it == metadata.metaListeners_.end()) {
