...
Feature: Schema Comment
  ...
  Scenario Outline: Space Comment
    ...
    Then the result should be, in any order:
      | Space          | Create Space                                                                                                                                                                             |
      | "<space_name>" | "CREATE SPACE `<space_name>` (partition_num = 100, replica_factor = 1, charset = utf8, collate = utf8_bin, vid_type = FIXED_STRING(8), atomic_edge = false) comment = '<space_comment>'" |
    When executing query:
      """
  ...
  Scenario Outline: Space Comment Not set
    ...
    Then the result should be, in any order:
      | Space                  | Create Space                                                                                                                                                         |
      | "test_comment_not_set" | "CREATE SPACE `test_comment_not_set` (partition_num = 100, replica_factor = 1, charset = utf8, collate = utf8_bin, vid_type = FIXED_STRING(8), atomic_edge = false)" |
    When executing query:
      """
  ...
  Scenario Outline: Space Comment Empty String
    ...
    Then the result should be, in any order:
      | Space                | Create Space                                                                                                                                                                    |
      | "test_comment_empty" | "CREATE SPACE `test_comment_empty` (partition_num = 100, replica_factor = 1, charset = utf8, collate = utf8_bin, vid_type = FIXED_STRING(8), atomic_edge = false) comment = ''" |
    When executing query:
      """
