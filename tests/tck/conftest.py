...
def executing_query(username, password, conn_pool_to_first_graph_service, query, exec_ctx, request):
    try:
        ...
        exec_ctx["result_set"] = "Fail to create a new session from connection pool, fail to authenticate, error: Bad username/password or clientIp not in ip whitelist"
        exec_ctx["ngql"] = query;
    finally:
        if 'sess' in locals():
            sess.release()
...
def raised_authenticate_error(error_msg, exec_ctx):
    ...
    assert (m), f'Could not find "{msg}" in "{res_msg}" when authenticate and execute query: "{ngql}"'
